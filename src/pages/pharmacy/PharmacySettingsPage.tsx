...
function WarehousesSection() {
  ...
  const { data: types = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("warehouse_types").select("id, name_ru").order("sort_order");
      return data || [];
    },
  });
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-card overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/50">
            <tr className="text-left">
              <th className="p-3">Название</th><th className="p-3">Тип</th><th className="p-3">Отделение</th>
              <th className="p-3">Статус</th><th className="p-3 w-12"></th>
            </tr>
          </thead>
          <tbody>
            ...
            {warehouses.length === 0 && (
              <tr><td colSpan={5} className="p-6 text-center text-muted-foreground">No warehouses.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="overflow-y-auto sm:max-w-md">
          <SheetHeader>
            <SheetTitle>{form.id ? "Edit Warehouse" : "Add Warehouse"}</SheetTitle>
          </SheetHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="wh-active" className="cursor-pointer">Активен</Label>
            </div>
          </div>
          <SheetFooter>
            <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
            <Button
              ...
              disabled={saveMutation.isPending || !form.name.trim() || !form.warehouse_type_id}
            >
              Save
            </Button>
          </SheetFooter>

                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    {s.schedule_type === "slots" && s.slot_duration_minutes && (
                      <span className="text-xs text-muted-foreground">{s.slot_duration_minutes} min slots</span>
                    )}
                  </div>
                  <div className="flex items-center gap-1 flex-wrap">
                    {DAYS.map((d) => (
                      <span key={d.value} className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${
...
export function ScheduleDialog({
  ...
  type RecurringBlock = { days: number[]; from: string; to: string; label: string };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (editing) {
        const { error } = await supabase.from("physician_schedules").update(payload).eq("id", editing.id);
        if (error) throw error;
        scheduleId = editing.id;
      } else {
        const { data, error } = await supabase.from("physician_schedules").insert(payload).select("id").single();
        if (error) throw error;
        scheduleId = data.id;
      }
      ...
      if (scheduleType === "slots") {
        ...
        const { data: slotsCreated, error: slotErr } = await supabase.rpc("generate_slots", {
          p_schedule_id: scheduleId, p_from_date: validFrom, p_to_date: toDate,
        });
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setScheduleType("slots")}
                ...
                <p className="text-xs text-muted-foreground mt-1">Patients book specific time slots in advance.</p>
              </button>
              <button type="button" onClick={() => setScheduleType("queue")}
                className={`rounded-lg border-2 p-4 text-left transition-colors ${
                ...
                <p className="text-xs text-muted-foreground mt-1">Patients receive a queue number on arrival.</p>
              </button>
            </div>
          </div>
          ...
          {scheduleType === "slots" && (
            <div className="space-y-1.5">
              ...
              <Input type="number" min={1} value={slotDuration} onChange={(e) => setSlotDuration(e.target.value)} />
            </div>
          )}
          ...
          <div className="space-y-2 border-t pt-4">
            ...
            <p className="text-xs text-muted-foreground">E.g. lunch breaks or prayer times that repeat on selected days.</p>
                ...
                {recurringBlocks.map((b, idx) => (
                  <div key={idx} className="rounded-md border p-3 space-y-2">
                    <div className="flex flex-wrap gap-1">
                      {DAYS.map((d) => (
                        <button type="button" key={d.value} onClick={() => toggleBlockDay(idx, d.value)}
                    ...
                    <div className="grid grid-cols-[1fr_1fr_2fr_auto] gap-2 items-end">
                      ...
                      <div className="space-y-1">
                        ...
                        <Input value={b.label} onChange={(e) => updateRecurringBlock(idx, { label: e.target.value })} placeholder="e.g. Lunch break" />
                      </div>
                      <Button type="button" size="sm" variant="outline" className="text-destructive" onClick={() => removeRecurringBlock(idx)}>
...
export default function ScheduleSection({ physicianId }: { physicianId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <BlocksPanel selection={selection} onAdd={() => setBlockOpen(true)} />
      {scheduleOpen && (
        <ScheduleDialog
          open={scheduleOpen}

-- Migration 066: Set completed_by on visit_services when document is confirmed
-- complete_document RPC was not setting completed_by on visit_services
-- causing null completed_by even after physician confirms document
...
BEGIN
  ...
  -- Update linked visit_service — now includes completed_by
  IF v_doc.visit_service_id IS NOT NULL THEN
  ...
  -- Activate hospitalization recommendation if exists

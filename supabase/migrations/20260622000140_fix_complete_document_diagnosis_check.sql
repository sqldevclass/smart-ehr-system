-- Migration 140: Fix complete_document RPC mandatory field check
--
-- Problem: mandatory fields in the "diagnosis" section
-- (attribute_code starting with "diag.") are stored in
-- patient_diagnoses, not patient_document_field_values.
-- The existing check always fails for these fields.
--
-- Fix:
-- 1. Exclude diagnosis-section mandatory fields from the
--    patient_document_field_values check.
-- 2. Instead, verify at least one patient_diagnoses row
--    exists for the document's scope (hospitalization or visit).
...
BEGIN
  ...
  -- Exclude fields in the "diagnosis" section — those are stored
  -- in patient_diagnoses, not patient_document_field_values.
  ...
  WHERE dtf.document_type_id = v_doc.document_type_id
    ...
    AND ds.code <> 'diagnosis'          -- skip diagnosis-section fields
    AND NOT EXISTS (
  ...
  -- ── 6. Validate diagnosis section if it has mandatory fields ──
  SELECT EXISTS (
    SELECT 1
    FROM public.document_type_fields dtf
    JOIN public.document_sections ds ON ds.id = dtf.section_id
  ...
  -- ── 7. Check signature count for multi-sig documents ────

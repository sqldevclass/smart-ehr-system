-- Migration 141: Remove criticality_flag gate from complete_document RPC
--
-- criticality_flag was added as a placeholder in migration 043
-- but no frontend UI was ever built to set it. It defaults to false
-- and blocks ALL document confirmation with no way to pass.
-- Removing the check until a proper clinical workflow is designed.
...
BEGIN
  ...
  -- criticality_flag check removed — flag defaults to false and
  -- no UI exists to set it. Re-add when clinical workflow is built.
  ...
  -- ── 4. Validate non-diagnosis mandatory fields ──────────
  ...
  -- ── 5. Validate diagnosis section if required ────────────
  SELECT EXISTS (
    SELECT 1
    FROM public.document_type_fields dtf
    JOIN public.document_sections ds ON ds.id = dtf.section_id

-- Migration 173: complete_document's auto-complete step required
-- document_types.linked_service_type_id to be set, but that column
-- has been NULL for every document type since it was introduced —
-- meaning the auto-complete has never fired for any document type,
-- for any service. The correct rule is simpler and needs no such
-- guard: any service with a linked document (services.
-- linked_document_type_id) should complete when that document is
-- confirmed, unconditionally — the visit_service_id on the
-- document already unambiguously identifies which specific service
-- to complete, regardless of what service_type it belongs to.
...
BEGIN
  ...
  -- ── 9. Complete the linked visit_service, unconditionally
  --      whenever this document has one — no service_type gate.

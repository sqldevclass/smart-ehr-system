-- Migration 088: Fix extend_prescription_to_date
-- Instead of generating slots from schedule_times,
-- clone the most recent day's active (non-skipped)
-- slots to the new target date.
...
BEGIN
  ...
  -- Check no slots already exist for target date
  IF EXISTS (
    SELECT 1
  ...
  -- Find the most recent day that has
  -- active (non-skipped) slots

-- Migration 118: Fix PRN flow
-- order_prn_drug: set preliminary (not in_progress)
-- dispense_prescription: accept preliminary for PRN

-- Fix order_prn_drug to set preliminary
...
BEGIN
  ...
  -- Set to preliminary so pharmacist can see and process it
  -- (same as regular prescription flow)
...
-- Fix dispense_prescription to accept preliminary status
-- (for both regular and PRN prescriptions)
...
BEGIN
  ...
  -- Accept both in_progress (regular) and preliminary (PRN after nurse orders)
  IF v_prescription.status_code NOT IN ('in_progress', 'preliminary') THEN
    RAISE EXCEPTION 'Prescription cannot be dispensed in status: %',
      v_prescription.status_code;
  END IF;

-- Migration 050: Fix complete_document to always update visit_service
-- when visit_service_id is set, regardless of linked_service_type_id.
-- The service already knows its document type via services.linked_document_type_id.
-- The document type does not need linked_service_type_id to complete the service.
...
BEGIN
  ...
  -- Always update visit_service if linked
  ...
  RETURN jsonb_build_object(
    ...
    'visit_service_updated', v_doc.visit_service_id IS NOT NULL
  );
